...
## faq
1. 类型收窄  
类型收窄即数据丢失精度或变化的隐式类型转换：
    + 浮点型转整型，丢失精度
    + 高精度浮点型转低精度(long double转double/double转float)，丢失精度
    + 整型转浮点型，整数大到浮点型无法精确表示，或无法表示。
    + 较长整型转较短整型，不能表示时。

如果变量a从A转成B，再从B转成A，其值不变，就不是类型收窄。
类型收窄一般会有警告或错误提示，常常是出问题的征兆。