...
函数的类型被称为函数的标识符，如果两个函数形参列表和返回值列表的变量类型一一对应，则这两个函数称为有相同的类型和标识符。
Go语言函数没有默认参数值。
函数调用者不关心函数声明的形参名，仅需依序提供实参列表。
...
把所有panic异常都恢复的做法会导致漏洞被忽略，并不可取，且不能保证包级变量是否与预期一致。
