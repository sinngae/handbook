...
## 分类
- 序列式容器，vector list queue
- 关联式容器，set map multiset multimap
- 配置式容器，queue stack priority_queue
- 无序管理式容器，
...
# trie/radix树
radix tree，基数树、字典树、前缀树，k路搜索树。
用于一组集合中搜索特定的键（通常是字符串，还可以是path），还可以为键保存对应的值（在任一节点中）。

+ 前缀树节点不保存完整的键，而是单个或多个字符
+ 完整的键，从根节点遍历道对应的节点，每个节点的字符连接起来，就是完整的键值
+ radix结构，当父节点只有一个子节点时，子节点合并到父节点
    + trie结构不做合并
+ 
...
linux内核中，基数树的键为整数类型用于，主要用于
+ 内存管理，页表管理（高效地虚实映射）、内存区域管理
+ 文件系统，索引节点inode管理、文件数据块管理
+ 网络协议栈，路由表管理、套接字管理
+ 设备驱动？

Gin 的radix tree Router。
