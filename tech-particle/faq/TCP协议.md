...
1.保证最后的ACK到达B端，如果B端重发FIN报文，处于TIME_WAIT的A端可以重发ACK（并重启MSL定时器，这样看来应该不是为了这个）。 
...
TCP是全双工的通讯，三次挥手做不到双方应用层控制主动关闭写。  
...
UDP更适合于消息的多播发布可以向多个点传送消息（因为没有繁琐的握手机制与挥手机制），同时UDP适用于快速传输的协议，对信息的时实性要求严格的协议。
