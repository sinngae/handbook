...
+ Architecture
    + 战略上的顶层设计蓝图
    + 回答“系统由哪些部分组成”和“各部分怎么协调”的问题。
    + 关注整体结构、模块划分、交互方式，以及肺功能需求（可扩展性、容错性、安全性）
+ Framework
    + 战术上的策略，可重复使用执行的代码骨架
    + 提供预定义的类库、接口和规范，帮助快速实现功能
    + 关注技术细节和开发效率
+ 架构是战略，框架是战术。
    + 架构为框架提供设计约束，框架为架构落地提供支撑。
    + 在论文中，Architecture 更适合描述设计理念与系统结构，Framework 更适合描述实现工具与代码复用。
    + 如果你的研究贡献在于设计方法，优先用 Architecture；如果贡献在于提供可复用实现，优先用 Framework。
