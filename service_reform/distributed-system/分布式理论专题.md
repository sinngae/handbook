...
＋ 主从复制、哨兵系统、一致性哈希，可选读写主节点
    ...
    ＋ P，多节点、节点可挂
+ Redis集群，AP
    + C，读写分离，只读从节点、多个从节点不一致
    + A，主从复制、故障转移，无单节点问题
    + P，一致性哈希，多节点、节点可挂

如果只读写主节点，那么CP弱A；如果写主读从，那么AP弱C。

## ZK CP
+ C，ZAB协议（Paxos协议的变种）强一致性协议
    + 超过半数以上节点写成功才返回
    + 所有读写按照全局顺序执行
    + 提供了顺序一致性、原子性、单系统快照等保障
+ 非A，无单节点问题，但超过一半以上则不可用（3挂1、5挂3）
+ P，多节点（节点可挂）

ZK牺牲了一部分可用性，确保了集群中的一部分容错和一部分一致性。再结合选举、查询取最新（复杂的节点间协调，成本比较高）达成了强一致性和一部分节点可挂。


## ETCD CP
+ C，ETCD 使用Raft一致性算法
    + leader选举，多数派提交，顺序一致性
+ 非A，如果脑裂为多个区，则少数区不可用
    + Leader选举障碍时，集群不可用（会议时刻）
+ P，多节点，可挂少数部分
...
+ etcd基于KV键值对
+ zk基于树形结构
+ 都有watch or watcher机制
+ 临时数据，etcd 租约机制 vs zk session机制
+ etcd性能更好，zk适合读多写少（服务自动注册和发现）
+ 
